...
export function HowItWorks() {
  const steps = [
    ["Fill in a short form on your phone", "We send you a private link. Add details and photos at your own pace. Your progress saves as you go."],
    ["We build and check your guide", "Your answers become a finished guide in your property's own colors. We read every page before it goes live."],
    ["Share the link or print the QR sign", "Each guide has its own address. Send it before arrival or leave the sign by the door."],
  ];
  return (
    <Section id="how">
      <div className="grid lg:grid-cols-[0.8fr_1.2fr] gap-10 lg:gap-20 items-start">
        ...
        <ol className="list-none m-0 p-0 border-t border-[#dcdcd9]">
          {steps.map(([title, body], i) => (
            <li key={title} className="grid grid-cols-[64px_1fr] py-7 border-b border-[#dcdcd9]">
              <span className="font-serif text-xl text-brand-navy">{i + 1}</span>
              <div>
                <b className="block text-lg mb-1.5">{title}</b>
                <p className="m-0 text-[#4a4a48] leading-relaxed">{body}</p>
              </div>
            </li>
          ))}
        </ol>
